...
AttentionImpl::AttentionImpl(int64_t dim, int64_t numHeads, double attnRatio) {
    ...
    _qkv = register_module("qkv", Conv(dim, h, 1, 1, std::nullopt, 1, 1, false));
    _proj = register_module("proj", Conv(dim, dim, 1, 1, std::nullopt, 1, 1, false));
    _pe = register_module("pe", Conv(dim, dim, 3, 1, std::nullopt, dim, 1, false));
}
...
PSABlockImpl::PSABlockImpl(int64_t c, double attnRatio, int64_t numHeads, bool shortcut) {
    ...
    ffn->push_back(Conv(c, c * 2, 1));
    ffn->push_back(Conv(c * 2, c, 1, 1, std::nullopt, 1, 1, false));
    _ffn = register_module("ffn", ffn);
...
PSAImpl::PSAImpl(int64_t c1, int64_t c2, double e) {
    ...
    torch::nn::Sequential ffn;
    ffn->push_back(Conv(_c, _c * 2, 1));
    ffn->push_back(Conv(_c * 2, _c, 1, 1, std::nullopt, 1, 1, false));
    _ffn = register_module("ffn", ffn);
}
