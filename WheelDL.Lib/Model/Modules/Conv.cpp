...
namespace WheelDL {
	namespace Model {
		namespace Modules {
			...
			UpsampleImpl::UpsampleImpl(double scaleFactor, const std::string& mode)
			{
				char firstChar = std::tolower(mode[0]);
				auto options = torch::nn::UpsampleOptions()
					.scale_factor(std::vector<double>{scaleFactor, scaleFactor});
				...
				if (firstChar == 'n') {
					...
					throw std::runtime_error("Unsupported Upsample mode: " + mode);
				}
				auto upsample = torch::nn::Upsample(options);
				_upsample = register_module("upsample", upsample);
			}

			torch::Tensor UpsampleImpl::forward(torch::Tensor x)
			{
				return _upsample->forward(x);
			}
			...
			MaxPool2dImpl::MaxPool2dImpl(int64_t k, int64_t s, std::optional<int64_t> p)
			{
				...
				_pool = register_module("pool", torch::nn::MaxPool2d(options));
			}

			torch::Tensor MaxPool2dImpl::forward(torch::Tensor x)
			{
				return _pool->forward(x);
			}
			...
			AvgPool2dImpl::AvgPool2dImpl(int64_t k, int64_t s, std::optional<int64_t> p)
			{
				// If stride is -1, use kernel_size as stride (PyTorch default behavior)
				int64_t actual_stride = (s == -1) ? k : s;
				int64_t padding = (actual_stride == k) ? 0 : autoPad(k, p, 1);
				...
				_pool = register_module("pool", torch::nn::AvgPool2d(options));
			}
			...
			GlobalAvgPoolImpl::GlobalAvgPoolImpl()
			{
				_pool = register_module("pool", torch::nn::AdaptiveAvgPool2d(torch::nn::AdaptiveAvgPool2dOptions(1)));
			}

			torch::Tensor GlobalAvgPoolImpl::forward(torch::Tensor x)
			{
				return _pool->forward(x);
			}

			// ============================================================================
			// NaiveConv Implementation
			// ============================================================================

			NaiveConvImpl::NaiveConvImpl(int64_t c1, int64_t c2, int64_t k, int64_t s,
				std::optional<int64_t> p, int64_t g, int64_t d)
			{
				// Validate input parameters
				if (c1 <= 0) {
					throw std::invalid_argument("NaiveConvImpl: c1 must be positive, got: " + std::to_string(c1));
				}
				if (c2 <= 0) {
					throw std::invalid_argument("NaiveConvImpl: c2 must be positive, got: " + std::to_string(c2));
				}
				if (k <= 0) {
					throw std::invalid_argument("NaiveConvImpl: k must be positive, got: " + std::to_string(k));
				}
				if (s <= 0) {
					throw std::invalid_argument("NaiveConvImpl: s must be positive, got: " + std::to_string(s));
				}
				if (g <= 0) {
					throw std::invalid_argument("NaiveConvImpl: g must be positive, got: " + std::to_string(g));
				}

				int64_t padding = autoPad(k, p, d);

				// Create Conv2d layer (no BatchNorm, no Activation)
				_conv = register_module("conv",
					torch::nn::Conv2d(torch::nn::Conv2dOptions(c1, c2, k)
						.stride(s)
						.padding(padding)
						.groups(g)
						.dilation(d)
						.bias(true)));  // bias enabled since no BatchNorm
			}

			torch::Tensor NaiveConvImpl::forward(torch::Tensor x)
			{
