...
public:
    ...
    /**
     * @brief Get head input indices (which layers the head takes input from)
     *
     * @return Vector of layer indices that feed into the head
     */
    std::vector<int64_t> getHeadInputIndices() const;
...
private:
    ...
    // Head-specific indices
    std::vector<int64_t> _headInputIndices;  // Which layers feed into the head
};
