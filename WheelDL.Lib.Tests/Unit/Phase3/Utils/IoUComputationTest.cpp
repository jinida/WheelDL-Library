...
TEST_F(IoUComputationTest, StandardIoU_BatchProcessing_ReturnsCorrectMatrix) {
	...
	EXPECT_NEAR(iou[0].item<float>(), 1.0f, tolerance);
	EXPECT_NEAR(iou[1].item<float>(), 1.0f, tolerance);
}
...
TEST_F(IoUComputationTest, GIoU_PartialOverlap_ReturnsCorrectValue) {
	...
	EXPECT_LE(giou.item<float>(), standard_iou.item<float>() - tolerance);
}
...
TEST_F(IoUComputationTest, Batch_MultipleBoxes_ReturnsCorrectMatrix) {
	...
	EXPECT_NEAR(iou[0].item<float>(), 1.0f, tolerance);  // Perfect match
	EXPECT_NEAR(iou[1].item<float>(), 1.0f, tolerance);  // Perfect match
	EXPECT_NEAR(iou[2].item<float>(), 0.0f, tolerance);  // No overlap
}
...
TEST_F(IoUComputationTest, Batch_LargeBatch_ProcessesEfficiently) {
	...
	auto boxes2 = torch::rand({ 1000, 4 }) * 100.0f;
